...
add_action( 'cs_init', array( 'BenutzerdefinierteSeitenleistenVisibility', 'instance' ) );
 ...
 * @since  2.0
 */
class BenutzerdefinierteSeitenleistenVisibility extends BenutzerdefinierteSeitenleisten {
	...
	public static function instance() {
		...
		if ( null === $Inst ) {
			$Inst = new BenutzerdefinierteSeitenleistenVisibility();
		}
	...
	protected function get_widget_data( $instance ) {
		...
		if ( null === $Condition_keys ) {
			...
			$type_list = BenutzerdefinierteSeitenleisten::get_post_types( 'objects' );
	...
	public function admin_widget_button( $widget, $return, $instance ) {
		...
		if ( false === $Loaded ) {
			...
			$pagetype_list = array(
				'frontpage' => __( 'Front Page', 'ps-sidebars' ),
				'home' => __( 'Post Index', 'ps-sidebars' ),
				'single' => __( 'Single Page', 'ps-sidebars' ),
				//'posts' => __( 'Posts page', 'ps-sidebars' ),  "Posts page" is same as "Post Index"...
				'archive' => __( 'Archives', 'ps-sidebars' ),
				'search' => __( 'Search Results', 'ps-sidebars' ),
				'e404' => __( 'Not Found (404)', 'ps-sidebars' ),
				'preview' => __( 'Preview', 'ps-sidebars' ),
				'day' => __( 'Archive: Day', 'ps-sidebars' ),
				'month' => __( 'Archive: Month', 'ps-sidebars' ),
				'year' => __( 'Archive: Year', 'ps-sidebars' ),
			);
		...
		<?php if ( ! isset( $_POST['csb-visibility-button'] ) ) : ?>
			<a href="#" class="button csb-visibility-button"><span class="dashicons dashicons-visibility"></span> <?php _e( 'Visibility', 'ps-sidebars' ); ?></a>
		<?php else : ?>
		...
		<div class="csb-option-row csb-action">
			<label for="<?php echo esc_attr( $widget->id ); ?>-action" class="lbl-show-if toggle-action" <?php if ( ! $action_show ) : ?>style="display:none"<?php endif; ?>><?php _e( '<b>Show</b> widget if:', 'ps-sidebars' ); ?></label>
			<label for="<?php echo esc_attr( $widget->id ); ?>-action" class="lbl-hide-if toggle-action" <?php if ( $action_show ) : ?>style="display:none"<?php endif; ?>><?php _e( '<b>Hide</b> widget if:', 'ps-sidebars' ); ?></label>
		...
		<div class="csb-option-row csb-always" <?php if ( ! $data['always'] ) : ?>style="display:none"<?php endif; ?>>
			<label><?php _e( 'Always', 'ps-sidebars' ); ?></label>
		</div>

		<?php /* DATE */ /* ?>
		<div class="csb-option-row csb-date" style="display:none">
			<label for="<?php echo esc_attr( $widget->id ); ?>-date">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'On these dates', 'ps-sidebars' ); ?>
			</label>
		...
		<div class="csb-option-row csb-guest" <?php if ( empty( $cond['guest'] ) ) : ?>style="display:none"<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-guest1" style="padding-top:10px;margin-bottom:0">
				...
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'User is not logged-in (Guest)', 'ps-sidebars' ); ?><br />
			</label>
			<label for="<?php echo esc_attr( $widget->id ); ?>-guest2" style="border:0;margin-bottom:0">
				...
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'User is logged-in (Member)', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
		</div>
		...
		<div class="csb-option-row csb-roles" <?php if ( empty( $cond['roles'] ) ) : ?>style="display:none"<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-roles">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'User has role', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
		...
		<div class="csb-option-row csb-membership" <?php if ( empty( $cond['membership'] ) ) : ?>style="display:none"<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-membership">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'User has Membership Level', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			...
			<?php foreach ( $membership_levels as $level ) : ?>
				...
				<option <?php selected( $is_selected ); ?> value="<?php echo esc_attr( $level['id'] ); ?>">
					...
					<?php if ( ! $level['level_active'] ) { _e( '(inactive)', 'ps-sidebars' ); } ?>
				</option>
			<?php endforeach; ?>
			</select>
		</div>
		...
		<div class="csb-option-row csb-membership2" <?php if ( empty( $cond['membership2'] ) ) : ?>style="display:none"<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-membership2">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'User has Membership', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			...
			<?php foreach ( $membership2_items as $item ) : ?>
				...
				<option <?php selected( $is_selected ); ?> value="<?php echo esc_attr( $item->id ); ?>">
					...
					<?php if ( ! $item->active ) { _e( '(inactive)', 'ps-sidebars' ); } ?>
				</option>
			<?php endforeach; ?>
			</select>
		</div>
		...
		<div class="csb-option-row csb-pagetypes" <?php if ( empty( $cond['pagetypes'] ) ) : ?>style="display:none"<?php endif; ?>>
			<label for="<?php echo esc_attr( $widget->id ); ?>-pagetypes">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'On these special pages', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
		...
		<div class="csb-option-row csb-posttypes"
			...
			<label for="<?php echo esc_attr( $widget->id ); ?>-posttypes">
				<span class="csb-and" style="display:none"><?php _e( 'AND', 'ps-sidebars' ); ?></span>
				<?php _e( 'On any page of these types', 'ps-sidebars' ); ?>
			</label>
			<i class="dashicons dashicons-trash clear-filter show-on-hover action"></i>
			...
			<?php foreach ( $type_list as $type_item ) :
				...
				$lbl_all = sprintf( __( 'Only for specific %s', 'ps-sidebars' ), $type_item->labels->name );
				$lbl_single = sprintf( __( 'Only these %s:', 'ps-sidebars' ), $type_item->labels->name );
